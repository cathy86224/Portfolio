...
export default class moo extends Component {
  render() {
    return (
      <React.Fragment>
        <div className="sub-header">• TherMOOstat</div>
        <p className="p1">
          ...
          <br />
          <i>Overview: </i>
          Historical and real-time building energy data for University of
          California Davis. The first step to saving energy is seeing how much
          you use.
          <br />
          <br />
          <i>Website link: </i>
        ...
        {TEMPERTURE.map((item, i) => (
          <img
            className={"ceed " + "tem " + TEMPERTURE_NAME[i]}
            key={item}
            src={item}
            alt={"TherMoostat " + item}
          />
        ))}
      </React.Fragment>
    );
